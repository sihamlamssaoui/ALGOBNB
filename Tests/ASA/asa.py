...
def initial_approval_program():
    ...
    asa_allowed_address = Return(And(get_address_of_sender.hasValue() , get_address_of_sender.value() == Gtxn[1].sender()))
    ...
    program = Cond(
        ...
        [Txn.application_args[0] == Bytes("asa_allowed_address"), asa_allowed_address]
    )
